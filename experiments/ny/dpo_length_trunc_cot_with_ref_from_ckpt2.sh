...
CUDA_VISIBLE_DEVICES=0 WANDB_PROJECT=arithmetic-dpo WANDB_MODE=online python run.py \
    --ref_model=True \
    --ref_model_path='out/test-llama-384-6-6-1024-COT-digits-11_14_/checkpoint-6000' \
    \
    \
    --architecture=llama \
...
CUDA_VISIBLE_DEVICES=0 WANDB_PROJECT=arithmetic-dpo WANDB_MODE=online python run.py \
    ...
    --learning_rate=5e-7 \
    --lr_scheduler_type='cosine_with_min_lr' \
    --lr_scheduler_kwargs='{"min_lr": 5e-8}' \
    --adam_beta2=0.99 \
    --warmup_ratio=0.05 \
    --logging_steps=20 \
    --eval_strategy="steps" \
